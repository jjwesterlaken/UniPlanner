...
20. **THE ICON SET, IN THE GENERATED PROJECT.** `mobile/ios` and
    ...
    - The icon on the home screen is Grace's notebook mark, at every
      size, with no transparent corners and no white box behind it.
      (It was the mortarboard until 12 September 2026; if a device
      still shows that one, the build predates the master.)
    ...
    **The desktop slots derive from the same master**, so a new master
    changes `desktop/build/icon.png` and `desktop/build/icon.ico` in
    the same run. That is deliberate: the PNG was byte-identical to the
    old master and generated by nothing, so the first master swap would
    otherwise have left the desktop build shipping the previous glyph
