...
import { decodePng, encodePng, resize, solid, blank, composite, encodeIco } from "./lib/png.mjs";
...
/**
 * Tracked: what electron-builder reads for the desktop app.
 *
 * THE DESKTOP ICON WAS BYTE-IDENTICAL TO THE MASTER AND SILENTLY STOPPED
 * BEING SO. It is not generated by anything, so replacing the master
 * left Windows, macOS and Linux shipping the previous glyph while the
 * web and the phones shipped the new one — two renderings with nothing
 * comparing them, which is the exact structural cause of the 2.3.8
 * rejection, one platform over. It derives here now, so it cannot.
 *
 * The `.ico` sizes are the seven the previous one carried; Windows picks
 * whichever it needs for the taskbar, the installer and Explorer.
 */
...
/** The Windows icon container: the same square, at seven sizes. */
function desktopIco(master) {
  return encodeIco(DESKTOP_ICO_SIZES.map((size) => ({ size, png: square(master, size) })));
}
...
export function deriveAll(masterBuffer, { includeNative = true } = {}) {
  ...
  /* VERBATIM, like the iOS slot and for the same reason: `encodePng`
     always writes RGBA, so re-encoding an opaque master would add an
     alpha channel it does not need. */
  out.push({ path: "desktop/build/icon.png", bytes: masterBuffer });
  out.push({ path: "desktop/build/icon.ico", bytes: desktopIco(master) });
...
export { WEB, STORE, DESKTOP_ICO_SIZES, ANDROID_DENSITIES, IOS_ICON, ANDROID_RES, MASTER, rootDir };
