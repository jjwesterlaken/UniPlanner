...
/* V7 NAMES THE THIRD PARTIES, and it is a rejection rather than a
   refinement: App Store build 3514249 was refused under guidelines
   5.1.1(i) and 5.1.2(i) because the screen said "a transcription
   service" and "a summarising service" without naming either. Apple
   requires the recipient named, in the app, before the first AI action
   — and says explicitly that the privacy policy alone is not enough.

   v7 also adds the two things the old screen had no answer for:
     - WHAT DECLINING COSTS, which is nothing but the AI features. The
       old gate had an Accept button and no other way out, so a student
       who did not want to agree was left on a blocking overlay. That is
       not consent, it is a trap with one exit.
     - WHICH FEATURES send what. "Text you supply" covered readings and
       photos as a category (v5, v6); a named-recipient screen has to be
       concrete about the material as well as the recipient.

   THE WORDING IS GRACE'S, 12 September 2026, and it is kept verbatim.
   THREE THINGS ARE INTERPOLATED rather than typed, and each renders her
   exact sentence today: the retention periods (from aiNotesRetention.js,
   so the promise cannot drift from what the server enforces), the
   country phrase, and the transcription company's name. The last two
   are the same rule as the first — "companies in the United States" and
   "neither we nor Groq keep it" are both TRUE of the current provider
   list and FALSE the moment it changes, so they follow the list instead
   of contradicting it later.

   NO PROVIDER NAME IS WRITTEN OUT HERE. A test asserts that, so this
   text cannot name a service the code does not use, or miss one it can.

   THE VERSION DID NOT MOVE for Grace's pass or for Deepgram's removal.
   v7 has never shipped — it is unmerged — so there is nobody holding an
   acceptance of the earlier draft to re-ask. The provider change is
   carried by the fingerprint regardless, which is the mechanism that
   exists so a wording version never has to be remembered for it. */
/**
 * The promise about the AUDIO, in one place.
 *
 * It is made TWICE to a student — on the consent screen and in iOS's own
 * microphone dialog — and `mobile/scripts/native-permissions.mjs` used to
 * type it out with a comment saying "that exact phrase is what a test
 * greps for in both files, so if one changes, change the other." That is
 * the restatement pattern admitting itself in a comment, and Grace's
 * wording pass is exactly the change it was waiting for: rewording the
 * consent broke the grep rather than the agreement.
 *
 * One constant, interpolated into both. A source guard asserts neither
 * file spells it out, because two copies and a grep is what this
 * replaces.
 *
 * AUDIO ONLY. The transcript has its own, longer retention, and a dialog
 * implying otherwise would be inaccurate — which is the one thing the
 * old test got right and is kept.
 */
...
export const CONSENT_TEXT = {
  ...
  intro:
    `The AI features send some of your work to companies in ${providerCountries()} to do what you ask. ` +
    "Here's exactly what goes where.",
  /* WHO, BY NAME, FIRST — the rejection was about these being absent,
     and a student reading top-down should meet the recipients before the
     caveats. One sentence per provider, from aiProviders.js. Grace's
     draft has no heading over them: the sentences follow the intro
     directly, which reads as the answer to "here's exactly what goes
     where" rather than as a new section. */
  providers: providerBullets(),
  bullets: [
  ...
  /* THE DECLINE PATH, stated as a fact about the app rather than as
     reassurance. It is true: every AI feature is additive, and the
  ...
  /* THE COMPACT FORM, for a panel rather than a tab.
     ...
     sight — three stacked copies of it on the Study tab would be worse
     than the gate it replaces. This is what stands where the controls
     would be: what the agreement is for, and the one control that opens
     the real screen. There is STILL ONLY ONE CONSENT SCREEN — this is a
     door to it, not a second, shorter version of it, because two
     wordings of the same disclosure is two things to keep true. */
  stubLine:
    `The AI study features need your agreement first — they send your work to companies in ${providerCountries()} to do what you ask.`,
  stubAction: "See what's sent, and who gets it",
  acceptLabel: "I agree — turn on the AI features",
...
/**
 * True when the signed-in user hasn't accepted the current consent.
 *
 * TWO REASONS TO RE-ASK, and the second is what Apple's requirement
 * actually needs. The editorial VERSION moves when the wording changes,
 * as it always has. The provider FINGERPRINT moves when the set of
 * third parties changes — and that one is derived from
 * `src/aiProviders.js` rather than remembered, so adding, removing or
 * renaming a recipient re-prompts every student whether or not anybody
 * thought to bump a number in the same commit.
 *
 * An acceptance recorded before fingerprints existed has none, and is
 * treated as not matching: a student who agreed to "a transcription
 * service" has not agreed to Groq by name, which is the whole point of
 * the bump.
 */
export function needsConsent(meta, version = AI_CONSENT_VERSION, fingerprint = providerFingerprint()) {
  const consent = meta && meta.aiConsent;
  if (!consent) return true;
  if ((consent.version || 0) < version) return true;
  return (consent.providers || "") !== fingerprint;
}

/** Patch to merge into `data.meta` when the user accepts. */
export function buildConsentPatch(
  version = AI_CONSENT_VERSION,
  nowISO = () => new Date().toISOString(),
  fingerprint = providerFingerprint()
) {
  return { aiConsent: { version, acceptedAt: nowISO(), providers: fingerprint } };
}
...
const ERROR_MESSAGES = {
  ...
  /* The client-side consent refusal (aiConsentState.js). Unreachable
     from a screen, for the same reason as its ai-text twin. */
  consent_required: "You haven't agreed to the AI features yet — nothing was sent. Open the AI tab to read what is sent and agree there.",
